...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer, epoch):
    ...
    for step, (batch, batch1, batch2, orig_batch, orig_batch1, orig_batch2) in enumerate(l):
        ...
        loss_anchor = (loss_anchor + loss_anchor_reverse)/2.0
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_nmol{}_moremorefeat'.format(args.data_folder, args.n_mol)
    ...
    # set up model
    gnn = GNNComplete(args.num_layer, args.emb_dim, JK=args.JK, drop_ratio=args.dropout_ratio, gnn_type=args.gnn_type)
