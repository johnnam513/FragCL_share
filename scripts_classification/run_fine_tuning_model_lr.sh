...
export dataset_list=(clintox bbbp bace tox21 sider muv hiv toxcast)
...
if [ "$mode" == "random" ]; then
    ...
    for dataset in "${dataset_list[@]}"; do
        ...
        export output_path=../output/"$folder"/"$dataset"_lr1e-4.out
#        export output_model_dir=../output/"$folder"/"$dataset"
        ...
        bash ./run_molecule_finetune.sh \
        --dataset="$dataset" --runseed="$seed" --eval_train --batch_size="$batch_size" --lr=0.0001\
        --dropout_ratio=0.5 \
