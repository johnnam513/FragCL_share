...
if [ "$mode" == "random" ]; then
    ...
    for dataset in "${dataset_list[@]}"; do
        export folder="$mode1"_"$mode2"/"$seed"
        ...
        bash ./run_molecule_finetune_double.sh \
    ...
    for dataset in "${dataset_list[@]}"; do
        export folder="$mode1"/"$mode2"/"$seed"
        mkdir -p ../output/"$folder"
        mkdir -p ../output/"$folder"/"$dataset"

        export output_path=../output/"$folder"/"$dataset".out
#        export output_model_dir=../output/"$folder"/"$dataset"
        export input_model_file1=../output/"$mode1"/pretraining_model.pth
        export input_model_file2=../output/"$mode2"/pretraining_model.pth

        #echo "$SLURM_JOB_ID"_"$SLURM_ARRAY_TASK_ID" > "$output_path"
        echo `date` >> "$output_path"

        bash ./run_molecule_finetune_mix.sh \
        --dataset="$dataset" --runseed="$seed" --eval_train --batch_size="$batch_size" \
        --dropout_ratio=0.5 \
        --input_model_file1="$input_model_file1" \
        --input_model_file2="$input_model_file2" \
        >> "$output_path"
