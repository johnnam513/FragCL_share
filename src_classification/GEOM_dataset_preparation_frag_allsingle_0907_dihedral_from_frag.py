...
def mol_to_graph_data_obj_simple_3D(mol):
    ...
    if len(mol.GetBonds()) > 0:  # mol has bonds
        ...
        for bond in mol.GetBonds():
            i = bond.GetBeginAtomIdx()
            j = bond.GetEndAtomIdx()
            #if allowable_features['possible_bonds'].index(bond.GetBondType()) == 0 and x[i][0].item() == 6 and x[j][0].item() == 5:
            #    cc_single_bond_list.append((i,j))
        ...
        min_diff = 1000
        #for (i,j) in cc_single_bond_list:
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_singlebond_manual_1207_sorted_dihedral_from_frag_nohydanchor'.format(data_folder, n_mol)
        ...
        Molecule3DDatasetFrag(root=root_2d, n_mol=n_mol,
                          smiles_copy_from_3D_file='/home/osikjs/GraphMVP/datasets/molecule_datasets/GEOM_2D_3D_nmol50000_4frag_cut_singlebond/processed/smiles.csv')
