...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer, epoch):
    ...
    for step, (batch, batch1, batch2, orig_batch, orig_batch1, orig_batch2) in enumerate(l):
        ...
        anchor1_ang = offset + orig_batch.anchor1_ang
        anchor2_ang = offset + orig_batch.anchor2_ang
        ...
        loss = 0
        if epoch < 10:
            loss += loss_2D + loss_3D + loss_anchor1 + loss_anchor2
        else:
            loss += CL_loss + loss_2D + loss_3D + loss_anchor1 + loss_anchor2
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_cut_singlebond_manual_0911_angles'.format(args.data_folder, args.n_mol)
