...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer, epoch):
    ...
    for step, (batch, batch1, batch2, orig_batch, orig_batch1, orig_batch2) in enumerate(l):
        ...
        node_rep, _, _ = molecule_model_2D.forward_cl_node(orig_batch.x, orig_batch.edge_index, orig_batch.edge_attr, orig_batch.batch)
        ...
        if args.model_3d == 'schnet':
            ...
            molecule_3D_repr_mixed, frag_3D_repr1, frag_3D_repr2 = molecule_model_3D.weighted_forward_frag_cl(orig_batch1.x[:,0], orig_batch1.positions + (torch.rand_like(orig_batch1.positions).cuda()), orig_batch1.batch, #+ torch.randn_like(orig_batch1.positions).cuda()
                                orig_batch2.x[:,0], orig_batch2.positions + (torch.rand_like(orig_batch2.positions).cuda()), orig_batch2.batch)       #+ torch.randn_like(orig_batch2.positions).cuda()
        ...
        offset =  torch.cat([torch.Tensor([0]).long().cuda(), torch.cumsum(binc, dim=0)[:-1]], dim=0).unsqueeze(1).repeat(1,3)
        offset_dihedral =  torch.cat([torch.Tensor([0]).long().cuda(), torch.cumsum(binc, dim=0)[:-1]], dim=0).unsqueeze(1).repeat(1,4)

        anchor1_ang = offset + orig_batch.anchor1
        anchor2_ang = offset + orig_batch.anchor2
        anchor_ang = offset_dihedral + orig_batch.dihedral_anchor
        # print(node_rep.shape)
        # print(anchor1_ang.max())
        # print(anchor2_ang.max())
        anchor1_tmp = torch.index_select(node_rep, 0, anchor1_ang.view(-1))
        
        anchor1_tmp2 = torch.reshape(anchor1_tmp, (batch_size,3,300)).reshape(batch_size,-1)
        anchor1_out = molecule_model_2D.forward_aux(anchor1_tmp2)
        loss_anchor1 = molecule_model_2D.ce(anchor1_out, batch.anchor1_angle)

        acc1 = (anchor1_out.argmax(dim=1) == batch.anchor1_angle).sum() / batch_size

        anchor2_tmp = torch.index_select(node_rep, 0, anchor2_ang.view(-1))
        
        anchor2_tmp2 = torch.reshape(anchor2_tmp, (batch_size,3,300)).reshape(batch_size,-1)
        anchor2_out = molecule_model_2D.forward_aux(anchor2_tmp2)
        loss_anchor2 = molecule_model_2D.ce(anchor2_out, batch.anchor2_angle)
        

        anchor_tmp = torch.index_select(node_rep, 0, anchor_ang.view(-1).long())
        
        anchor_tmp2 = torch.reshape(anchor_tmp, (batch_size,4,300)).reshape(batch_size,-1)
        anchor_out = molecule_model_2D.forward_aux2(anchor_tmp2)
        ...
        CL_acc_accum += CL_acc
        AE_loss_accum += loss_anchor1.detach().cpu().item()
        AE_acc_accum += acc1

        loss = 0
        if epoch < 10:
            loss += loss_2D + loss_3D + loss_anchor1 + loss_anchor2 + loss_anchor
        else:
            loss += CL_loss + loss_2D + loss_3D + loss_anchor1 + loss_anchor2 + loss_anchor
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_cut_singlebond_manual_0911_dihedral_fixed'.format(args.data_folder, args.n_mol)
