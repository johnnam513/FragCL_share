...
from datasets import Molecule3DDatasetFragRandomaug3d_brics
...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer):
    ...
    if args.verbose:
        ...
        l = loader
    for step, (batch, orig_batch, frag0, frag1, frag2, frag3, frag4, frag5, frag6, frag7, frag8, frag9) in enumerate(l):
        batch = batch.to(device)
        orig_batch = orig_batch.to(device)
        frag_list_before = [frag0, frag1, frag2, frag3, frag4, frag5, frag6, frag7, frag8, frag9]
        frag_list = []
        for frag in frag_list_before:
            frag_list.append(frag.to(device))


        molecule_2D_repr = molecule_model_2D.forward_cl(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
        molecule_2D_repr_mixed = molecule_model_2D.forward_frag_cl(frag_list)       
        ...
        #    molecule_3D_repr_mixed = molecule_model_3D.forward_frag_cl(frag_list)       #+ torch.randn_like(orig_batch2.positions).cuda()
        ...
        loss = 0
        if args.alpha_1 > 0:
            loss += loss_2D#CL_loss * args.alpha_1 + loss_2D#+ loss_3D
        #if args.alpha_2 > 0:
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/molecule_datasets/GEOM_2D_3D_nmol{}_brics'.format(args.data_folder, args.n_mol)
        dataset = Molecule3DDatasetFragRandomaug3d_brics(root=root_2d, n_mol=n_mol, choose=args.choose,
