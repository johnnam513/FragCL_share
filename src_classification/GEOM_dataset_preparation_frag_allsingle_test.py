...
class Molecule3DDatasetFrag(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            data_smiles_list0 = list()
            data_smiles_list1 = list()
            data_smiles_list2 = list()
            # load 3D structure
            dir_name = '{}/rdkit_folder'.format(data_folder)
            drugs_file = '{}/summary_drugs.json'.format(dir_name)
            with open(drugs_file, 'r') as f:
            ...
            for smiles in tqdm(data_smiles_list):
                ...
                with open(mol_path, 'rb') as f:
                    ...
                    if mols is None:
                        ...
                        data_smiles_list1.append(Chem.MolToSmiles(mols[0]))
                        data2 = mol_to_graph_data(mols[1])
                        data_smiles_list2.append(Chem.MolToSmiles(mols[1]))
                        data_smiles_list0.append(smiles)
        ...
        data_smiles_series = pd.Series(data_smiles_list0)
        data_smiles_series1 = pd.Series(data_smiles_list1)
        data_smiles_series2 = pd.Series(data_smiles_list2)
        saver_path = join(self.processed_dir, 'smiles.csv')
        saver_path1 = join(self.processed_dir, 'smiles1.csv')
        saver_path2 = join(self.processed_dir, 'smiles2.csv')
        print('saving to {}'.format(saver_path))
        data_smiles_series.to_csv(saver_path, index=False, header=False)
        data_smiles_series1.to_csv(saver_path1, index=False, header=False)
        data_smiles_series2.to_csv(saver_path2, index=False, header=False)
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_nmol{}_cut_singlebond_test'.format(data_folder, n_mol)
