...
def mol_to_graph_data_obj_simple_3D_gen(mol):
    ...
    #print(x.shape, edge_index.shape, edge_attr.shape, positions.shape)
...
def dummy_3d_data():
    ...
    positions = torch.Tensor([0.0,0.0,0.0]).unsqueeze(0)
    x = torch.Tensor([0,0]).unsqueeze(0).type(torch.LongTensor)

    return Data(x=x, edge_index=edge_index,
                edge_attr=edge_attr, positions=positions)
#edge_index = torch.empty((2,0), dtype=torch.long)
#edge_attr = torch.empty((0,2), dtype=torch.long)
#positions = torch.empty((0,2), dtype=torch.float)
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            for smiles in tqdm(data_smiles_list):
                ...
                with open(mol_path, 'rb') as f:
                    ...
                    if mols is None:
                        data0 = mol_to_graph_data_obj_simple_3D(rdkit_mol)
                        data1 = dummy_3d_data()
                        data2 = dummy_3d_data()
                        data3 = dummy_3d_data()
                        ...
                        if mols[0] is None or mols[1] is None:
                            ...
                            try:
                                ...
                                data0 = mol_to_graph_data_obj_simple_3D_gen(mol0_0)
                                data1 = mol_to_graph_data_obj_simple_3D_gen(mol0_1)
                                data2 = mol_to_graph_data_obj_simple_3D_gen(mol1_0)
                                data3 = mol_to_graph_data_obj_simple_3D_gen(mol1_1)
                                ...
                                try:
                                    data0 = mol_to_graph_data_obj_simple_3D_gen(mols[0])
                                    data1 = mol_to_graph_data_obj_simple_3D_gen(mols[1])
                                    data2 = dummy_3d_data()
                                    data3 = dummy_3d_data()
                                except:
                                    data0 = mol_to_graph_data_obj_simple_3D(rdkit_mol)
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_4frag_cut_singlebond_app'.format(data_folder, n_mol)
        ...
        Molecule3DDatasetFrag(root=root_2d, n_mol=n_mol,
                          smiles_copy_from_3D_file='../datasets/molecule_datasets/smiles.csv')
