...
from datasets import Molecule3DDatasetFrag
...
def train(loader, model, optimizer, device):
    ...
    model.train()
    train_loss_accum = 0
    aux_criterion = torch.nn.BCELoss()
    for step, (batch, batch1, batch2) in enumerate(loader):
        ...
        #x, pred_p = model.forward_cl_aux(batch.x, batch.edge_index,
        #                      batch.edge_attr, batch.batch, batch.positions)
        ...
        x2 = model.forward_frag_cl(batch1.x, batch1.edge_index,
                              batch1.edge_attr, batch1.batch,
                              batch2.x, batch2.edge_index,
                              batch2.edge_attr, batch2.batch)
        #aux_loss = aux_criterion(pred_p, batch.positions.float())
        #print(aux_loss)
        loss = model.loss_cl(x, x2) #+ 0.5*aux_loss
...
if __name__ == "__main__":
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_nmol{}_aux'.format(args.data_folder, args.n_mol)
        dataset = Molecule3DDatasetFrag(root=root_2d, n_mol=n_mol,
