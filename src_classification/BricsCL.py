...
from pretrain_JOAO import bricscl
...
from datasets import Molecule3DDatasetBrics
...
class Collater:
    ...
    def __call__(self, batch):
        ...
        elem2 = []
        for i in range(bs):
            e1, e2 = batch[i][0], batch[i][1]
            elem1.append(e1)
            # elem2.append(e2)
            elem2.append(Batch.from_data_list(e2, self.follow_batch, self.exclude_keys))
        return Batch.from_data_list(elem1, self.follow_batch, self.exclude_keys), elem2
        # elem = batch[0]
...
def train(loader, model, optimizer, device):
    ...
    for step, (batch, batch1) in enumerate(loader):
        batch = batch.to(device)
        x = model.forward_all(batch.x, batch.edge_index, batch.edge_attr, batch.batch)

        # positive representation generation
        bs = len(batch1)
        x_new = torch.zeros((bs, x.shape[1])).to(device)
        for i in range(bs):
            brics = batch1[i].to(device)
            brics = model.forward_pool(brics.x, brics.edge_index, brics.edge_attr, brics.batch)
            brics = torch.mean(brics, dim = 0)
            x_new[i] = model.forward_project(brics).view(-1)

        # for i in range(bs):
        #     brics_list = batch1[i]
        #     if len(brics_list) == 1:
        #         brics = brics_list[0].to(device)
        #         x_new[i] = model.forward_all(brics.x, brics.edge_index, brics.edge_attr, None)[0]

        #     else:
        #         brics_rep = 0
        #         for j in range(len(brics_list)):
        #             brics = brics_list[j].to(device)
        #             brics_rep += model.forward_pool(brics.x, brics.edge_index, brics.edge_attr, None)/len(brics_list)
        #         x_new[i] = model.forward_project(brics_rep)[0]

        loss = model.loss_cl(x, x_new)
...
if __name__ == "__main__":
    ...
    if 'GEOM' in args.dataset:
        ...
        dataset = Molecule3DDatasetBrics(root=root_2d, n_mol=n_mol, choose=args.choose,
