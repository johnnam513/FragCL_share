...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer, epoch):
    ...
    for step, (batch, batch1, batch2, orig_batch, orig_batch1, orig_batch2) in enumerate(l):
        ...
        anchor_tail = torch.cat([anchor_from_frag1[:,0,:], anchor_from_frag2[:,1,:]], dim=1)
        anchor_inner = torch.cat([anchor_from_frag1[:,1,:], anchor_from_frag2[:,0,:]], dim=1)
        
        anchor_tail = anchor_tail.reshape(batch_size,-1)
        anchor_tail_out = molecule_model_2D.aux_tail(anchor_tail)
        
        anchor_inner = anchor_inner.reshape(batch_size,-1)
        anchor_inner_out = molecule_model_2D.aux_inner(anchor_inner)

        anchor_aug = torch.cat([anchor_tail_out, anchor_inner_out], dim=1)
        anchor_aug_out = molecule_model_2D.aux_aug(anchor_aug)

        loss_anchor_aug = molecule_model_2D.ce2(anchor_aug_out, batch.dihedral_angle)
        
        anchor_tail_reverse = torch.cat([anchor_from_frag2[:,1,:], anchor_from_frag1[:,0,:]], dim=1)
        anchor_inner_reverse = torch.cat([anchor_from_frag2[:,0,:], anchor_from_frag1[:,1,:]], dim=1)

        anchor_tail_reverse = anchor_tail_reverse.reshape(batch_size,-1)
        anchor_tail_reverse_out = molecule_model_2D.aux_tail(anchor_tail_reverse)

        anchor_inner_reverse = anchor_inner_reverse.reshape(batch_size,-1)
        anchor_inner_reverse_out = molecule_model_2D.aux_inner(anchor_inner_reverse)

        anchor_reverse_aug = torch.cat([anchor_tail_reverse_out, anchor_inner_reverse_out], dim=1)
        anchor_aug_reverse_out = molecule_model_2D.aux_aug(anchor_reverse_aug)

        dihedral_angle2 = torch.remainder(batch.dihedral_angle + 9, 18).long()
        loss_anchor_reverse_aug = molecule_model_2D.ce2(anchor_aug_reverse_out, dihedral_angle2)


        
        acc = (anchor_aug_out.argmax(dim=1) == batch.dihedral_angle).sum() / batch_size
        acc2 = (anchor_aug_reverse_out.argmax(dim=1) == dihedral_angle2).sum() / batch_size
        acc = (acc + acc2) /2.0

        loss_anchor = (loss_anchor_aug + loss_anchor_reverse_aug)/2.0
        ...
        loss = 0
        if epoch < 5:
            loss += loss_2D + loss_3D #+  loss_anchor/2.0
        else:
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_cut_singlebond_manual_0911_dihedral_from_frag'.format(args.data_folder, args.n_mol)
