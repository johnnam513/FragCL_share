...
def mol_fragment_brics(mol):
    return list(BRICS.BRICSDecompose(mol, returnMols=True))
...
def mol_to_graph_data(mol):
    ...
    if len(mol.GetBonds()) > 0:  # mol has bonds
        ...
        for bond in mol.GetBonds():
            ...
            edge_feature = [allowable_features['possible_bonds'].index(bond.GetBondType())] + \
                           [allowable_features['possible_bond_dirs'].index(bond.GetBondDir())]
            edges_list.append((i-ref_i, j-ref_j))
            edge_features_list.append(edge_feature)
            edges_list.append((j-ref_j, i-ref_i))
            edge_features_list.append(edge_feature)
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            for smiles in tqdm(data_smiles_list):
                ...
                with open(mol_path, 'rb') as f:
                    ...
                    rdkit_mol = Chem.MolFromSmiles(Chem.MolToSmiles(Chem.MolFromSmiles(Chem.MolToSmiles(conformer['rd_mol']))))
                    mols = mol_fragment_brics(rdkit_mol)
                    #fp = Chem.RDKFingerprint(rdkit_mol)
                    brics_data = []
                    #try:
                    for mol in mols:
                        brics_data.append(mol_to_graph_data(mol))
                    if self.pre_filter is not None:
                        brics_data = [data for data in brics_data if self.pre_filter(data)]

                    if self.pre_transform is not None:
                        brics_data = [self.pre_transform(data) for data in brics_data]
                    
                    brics_smiles_list = [Chem.MolToSmiles(x,True) for x in mols]
                    data_smiles_series = pd.Series(brics_smiles_list)
                    saver_path = join(self.processed_dir, f'smiles_{idx}.csv')
                    print('saving to {}'.format(saver_path))
                    data_smiles_series.to_csv(saver_path, index=False, header=False)
                    data, slices = self.collate(brics_data)
                    torch.save((data, slices), self.processed_paths[0] + "_" + str(idx))
                #except:
                    #    print('exception')
                    #    continue
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_nmol{}_brics_exclude_dummy_check'.format(data_folder, n_mol)
        ...
        Molecule3DDatasetFrag(root=root_2d, n_mol=n_mol,
                          smiles_copy_from_3D_file='..//datasets/GEOM_2D_3D_nmol50000_anybond_manual_1110_sorted_dihedral_from_frag_nohydanchor/processed/smiles.csv')
