...
def train(args, molecule_model_2D, molecule_model_3D, device, loader, optimizer, epoch):
    ...
    for step, (batch, batch1, batch2, orig_batch, orig_batch1, orig_batch2) in enumerate(l):
        ...
        anchor1 = anchor1.reshape(batch_size,-1)
        anchor1_out = molecule_model_2D.forward_aux(anchor1)
        ...
        anchor2 = anchor2.reshape(batch_size, -1)
        anchor2_out = molecule_model_2D.forward_aux(anchor2)
        ...
        acc = (anchor1_out.argmax(dim=1) == batch.anchor1_angle).sum() / batch_size
