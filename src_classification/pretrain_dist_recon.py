...
class Edge_Projector(nn.Module):
    def __init__(self):
        super(Projector, self).__init__()
        self.projection_head = nn.Linear(600,1)
    def forward(self, x):
        return self.projection_head(x)
...
def train(args, molecule_model_2D, device, loader, optimizer):
    ...
    #edge_criterion = nn.BCEWithLogitLoss()
    ...
    for step, batch in enumerate(l):
        ...
        for b in range(len(molecule_size)):
            ...
            dist_mat = torch.sqrt(a2 + b2 - 2*ab).squeeze()
            
            features = node_repr[mol_idx : mol_idx + molecule_size[b],:]
            
            features_1 = features.repeat(molecule_size[b],1).reshape(molecule_size[b], molecule_size[b], -1)
            features_2 = features.repeat_interleave(molecule_size[b], dim=0).reshape(molecule_size[b], molecule_size[b], -1)
            ...
            #pred_edge_mat = edge_projector(features).squeeze()
            #pred_edge_mat = pred_edge_mat + pred_edge_mat.transpose(0,1)
            #final_pred_edge_mat = torch.triu(pred_dist_mat, diagonal=1)
        ...
        torch.nn.utils.clip_grad_norm_(projector.parameters(), 5)
        optimizer.step()
    ...
    temp_loss = CL_loss_accum + dist_loss_accum
    if temp_loss < optimal_loss:
        optimal_loss = temp_loss
        save_model(save_best=True)
    print('CL Loss: {:.5f}\tCL Acc: {:.5f}\t\tdist Loss: {:.5f}\tTime: {:.5f}'.format(
        CL_loss_accum, CL_acc_accum, dist_loss_accum, time.time() - start_time))
    return
...
#        raise Exception
    ...
    model_param_group.append({'params': projector.parameters(), 'lr': args.lr})
