...
def mol_fragment(mol):
    ...
    if balanced_rxn is not None:
        ...
        p1 = FragmentMatcher()
        p1.Init(balanced_rxn[0])
        p1_match = p1.GetMatches(mol, uniquify=1)
        #print(len(p1_match))
        p2 = FragmentMatcher()
        p2.Init(balanced_rxn[1])
        p2_match = p2.GetMatches(mol, uniquify=1)
        match_dict = {}
        out = 0
        if len(p1_match) != 1 or len(p2_match) != 1:
            return None
        #print(len(p1_match), len(p2_match))
        #for i in range(len(p1_match)):
        #    for j in range(len(p2_match)):
        #        try:
        try:
            for atom in mol1.GetAtoms():
                match_dict[p1_match[0][atom.GetIdx()]] = atom.GetNumImplicitHs() + atom.GetNumExplicitHs()
            for atom in mol2.GetAtoms():
                #if p2_match[j][atom.GetIdx()] in match_dict.keys():
                #    continue
                match_dict[p2_match[0][atom.GetIdx()]] = atom.GetNumImplicitHs() + atom.GetNumExplicitHs()
            
            cnt = 0
            for atom in mol.GetAtoms():
                if match_dict[atom.GetIdx()] != atom.GetNumImplicitHs() + atom.GetNumExplicitHs():
                    atom.SetProp('delete', '1')
                    cnt += 1
                else:
                    atom.SetProp('delete', '0')
        except:
            return None
        if cnt != 2:
            #print("What?")
...
def mol_to_graph_data(mol, original):
    """
    Converts rdkit mol object to graph Data object required by the pytorch
    geometric package. NB: Uses simplified atom and bond features, and represent as indices
    :param mol: rdkit mol object
    ...
    for atom in mol.GetAtoms():
        ...
        atom_features_list.append(atom_feature)
        if original:
            delete_list.append([int(atom.GetProp("delete"))])
    x = torch.tensor(np.array(atom_features_list), dtype=torch.long)
    if original:
        positions = torch.tensor(np.array(delete_list), dtype=torch.long)
    # bonds, two features: bond type, bond direction
    if len(mol.GetBonds()) > 0:  # mol has bonds
        edges_list = []
        edge_features_list = []
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            for smiles in tqdm(data_smiles_list):
                ...
                with open(mol_path, 'rb') as f:
                    ...
                    #print(Chem.MolFromSmiles(Chem.MolToSmiles(Chem.MolFromSmiles(Chem.MolToSmiles(conformer['rd_mol'])))))
                    ...
                    if mols is None:
                        ...
                        data = mol_to_graph_data(mols[0], original=True)
                        data1 = mol_to_graph_data(mols[1], original=False)
                        data2 = mol_to_graph_data(mols[2], original=False)
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_nmol{}_aux'.format(data_folder, n_mol)
