...
class GNN_graphpred_mix(nn.Module):
    """
    ...
    def __init__(self, args, num_tasks, molecule_model1=None, molecule_model2=None):
        super(GNN_graphpred_mix, self).__init__()

        if args.num_layer < 2:
            raise ValueError("# layers must > 1.")

        self.molecule_model1 = molecule_model1
        self.molecule_model2 = molecule_model2
    ...
    def from_pretrained(self, model_file1, model_file2):
        self.molecule_model1.load_state_dict(torch.load(model_file1))
        self.molecule_model2.load_state_dict(torch.load(model_file2))
        return
    ...
    def forward(self, *argv):
        ...
        node_representation1 = self.molecule_model1(x, edge_index, edge_attr)
        graph_representation1 = self.pool(node_representation1, batch)

        node_representation2 = self.molecule_model2(x, edge_index, edge_attr)
        graph_representation2 = self.pool(node_representation2, batch)
        graph_representation = torch.cat((graph_representation1, graph_representation2), dim=1)
