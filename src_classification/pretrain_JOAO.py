...
class bricscl(nn.Module):

    def __init__(self, gnn):
        super(bricscl, self).__init__()
        self.gnn = gnn
    ...
    def forward_all(self, x, edge_index, edge_attr, batch):
...
class fragcl4(nn.Module):

    def __init__(self, gnn):
        super(fragcl4, self).__init__()
        self.gnn = gnn
        self.pool = global_mean_pool
        self.projection_head = nn.Sequential(
            nn.Linear(300, 300),
    ...
    def loss_cl4(self, x1, x2, frag1, frag2, frag3, frag4, frag5, frag6):
        T = 0.1
        batch, _ = x1.size()
        x1_abs = x1.norm(dim=1)
        x2_abs = x2.norm(dim=1)
        ...
        frag4_abs = frag4.norm(dim = 1)
        frag5_abs = frag5.norm(dim = 1)
        frag6_abs = frag6.norm(dim = 1)
        ...
        frag4_matrix = torch.einsum('ik,jk->ij', x1, frag4) / \
                     torch.einsum('i,j->ij', x1_abs, frag4_abs)
        frag5_matrix = torch.einsum('ik,jk->ij', x1, frag5) / \
                     torch.einsum('i,j->ij', x1_abs, frag5_abs)
        frag6_matrix = torch.einsum('ik,jk->ij', x1, frag6) / \
                     torch.einsum('i,j->ij', x1_abs, frag6_abs)
        ...
        frag4_matrix = torch.exp(frag4_matrix / T)
        frag5_matrix = torch.exp(frag5_matrix / T)
        frag6_matrix = torch.exp(frag6_matrix / T)
        ...
        neg4 = frag4_matrix[range(batch), range(batch)]
        neg5 = frag5_matrix[range(batch), range(batch)]
        neg6 = frag6_matrix[range(batch), range(batch)]
        pos_sim = sim_matrix[range(batch), range(batch)]
        loss = pos_sim / (sim_matrix.sum(dim=1) + neg1 + neg2 + neg3 + neg4 + neg5 + neg6 - pos_sim)
        
        loss = - torch.log(loss).mean()
        return loss

    def loss_cl2(self, x1, x2, frag1, frag2):
        T = 0.1
        batch, _ = x1.size()
        x1_abs = x1.norm(dim=1)
        x2_abs = x2.norm(dim=1)
