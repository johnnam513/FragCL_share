...
from models import GNN, GNN_graphpred_mix
...
if __name__ == '__main__':
    ...
    # set up model
    molecule_model1 = GNN(num_layer=args.num_layer, emb_dim=args.emb_dim,
                         JK=args.JK, drop_ratio=args.dropout_ratio,
                         gnn_type=args.gnn_type)
    
    molecule_model2 = GNN(num_layer=args.num_layer, emb_dim=args.emb_dim,
                         JK=args.JK, drop_ratio=args.dropout_ratio,
                         gnn_type=args.gnn_type)
    model = GNN_graphpred_mix(args=args, num_tasks=num_tasks,
                          molecule_model1=molecule_model1, molecule_model2=molecule_model2)

    
    if not args.input_model_file1 == '' and not args.input_model_file2 == '':
        model.from_pretrained(args.input_model_file1, args.input_model_file2)
    model.to(device)
    print(model)
    ...
    model_param_group = [{'params': model.molecule_model1.parameters()},
                         {'params': model.graph_pred_linear.parameters(),
                          'lr': args.lr * args.lr_scale},
                          {'params': model.molecule_model2.parameters()}]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_roc > best_val_roc:
            ...
            if not args.output_model_dir == '':
                ...
                saved_model_dict = {
                    'molecule_model1': molecule_model1.state_dict(),
                    'molecule_model2': molecule_model2.state_dict(),
