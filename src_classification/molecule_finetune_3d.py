...
class graphcl(nn.Module):
    ...
    def forward(self, x, edge_idx, edge_attr):
        return self.gnn(x, edge_idx, edge_attr)
...
if __name__ == '__main__':
    ...
    # tmp
    molecule_model = graphcl(molecule_model)
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(train_loader):
            batch = batch.to(device)
            representations.append(model.get_graph_representation(batch.x, batch.edge_index, batch.edge_attr, batch.batch)[0])
            label.append(batch.y.unsqueeze(1).to(torch.float64))
        torch.save(torch.cat(representations, dim=0), args.input_model_file[:-21] + args.dataset + '_init_representations')
        torch.save(torch.cat(label, dim=0), args.input_model_file[:-21] + args.dataset + '_labels')
