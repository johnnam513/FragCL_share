...
from pretrain_JOAO_addpool import graphcl
from torch_geometric.data import DataLoader

from datasets import MoleculeDataset_graphclDouble
...
def train(loader, model, optimizer, device):
    ...
    for step, (_, first_batch1, first_batch2, _, second_batch1, second_batch2) in enumerate(loader):
        ...
        first_x1 = model.forward_cl(first_batch1.x, first_batch1.edge_index,
                              first_batch1.edge_attr, first_batch1.batch)
        first_x2 = model.forward_cl(first_batch2.x, first_batch2.edge_index,
                              first_batch2.edge_attr, first_batch2.batch)
        loss1 = model.loss_cl(first_x1[:,:200], first_x2[:,:200])

        second_x1 = model.forward_cl(second_batch1.x, second_batch1.edge_index,
                              second_batch1.edge_attr, second_batch1.batch)
        second_x2 = model.forward_cl(second_batch2.x, second_batch2.edge_index,
                              second_batch2.edge_attr, second_batch2.batch)
        loss2 = model.loss_cl(second_x1[:,100:], first_x2[:,100:])
...
if __name__ == "__main__":
    ...
    if 'GEOM' in args.dataset:
        dataset = MoleculeDataset_graphclDouble('../datasets/{}/'.format(args.dataset),
