...
class Molecule3DDatasetBrics(MoleculeDataset):
    ...
    def __init__(self, root, n_mol,choose = 0,transform=None, seed=777,
        ...
        super(Molecule3DDatasetBrics, self).__init__(
    ...
    def get(self, idx):
        ...
        if self.brics_slice[idx]['x'] == None:
            brics_datas = [Data()]
            for key in self.data.keys:
                item, slices = self.data[key], self.slices[key]
                s = list(repeat(slice(None), item.dim()))
            ...
            brics_datas = [Data() for _ in range(len(self.brics_slice[idx]['x'])-1)]
            for i in range(len(self.brics_slice[idx]['x'])-1):
        ...
        if self.aug_mode == 'no_aug':
            ...
            for i in range(len(brics_datas)):
                brics_datas[i] = self.augmentations[n_aug](brics_datas[i].clone())
            ...
            for i in range(len(brics_datas)):
                n_aug = np.random.choice(2,1)[0]
                brics_datas[i] = two_augmentations[n_aug](brics_datas[i].clone())
