...
class MoleculeDataset_mae(InMemoryDataset):
    def __init__(self, root, dataset='zinc250k', transform=None,
        ...
        super(MoleculeDataset_mae, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self):
        ...
        def shared_extractor(smiles_list, rdkit_mol_objs, labels):
            ...
            for i in range(len(smiles_list)):
                ...
                #mol0, mol1 = mol_fragment(rdkit_mol)
                try:
                    mol0, mol1 = mol_fragment(rdkit_mol)
                    try:
                        mol0_0, mol0_1 = mol_fragment(mol0)
                        mol1_0, mol1_1 = mol_fragment(mol1)
                        data0 = mol_to_graph_data_obj_simple(mol0_0)
                        data1 = mol_to_graph_data_obj_simple(mol0_1)
                        data2 = mol_to_graph_data_obj_simple(mol1_0)
                        data3 = mol_to_graph_data_obj_simple(mol1_1)
                        ...
                        error += 1
                        data0 = mol_to_graph_data_obj_simple(mol0)
                        data1 = mol_to_graph_data_obj_simple(mol1)
                        data0_list.append(data0)
                        data1_list.append(data1)
                        data2_list.append(dummy_2d_data())
                        data3_list.append(dummy_2d_data())
                except:
                    error += 1
                    data0_list.append(mol_to_graph_data_obj_simple(rdkit_mol))
                    data1_list.append(dummy_2d_data())
                    data2_list.append(dummy_2d_data())
                    data3_list.append(dummy_2d_data())
            ...
            print(error)
            return data_list, data0_list, data1_list, data2_list, data3_list, data_smiles_list
        print(self.dataset)
        if self.dataset == 'zinc_standard_agent':
            ...
            data_list, data0_list, data1_list, data2_list, data3_list, data_smiles_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)
