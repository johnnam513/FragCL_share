...
class Molecule3DDatasetFragRandomaug3dRandcut(MoleculeDataset):
    ...
    def __init__(self, root, n_mol,choose = 0,transform=None, seed=777,
        ...
        super(Molecule3DDatasetFragRandomaug3dRandcut, self).__init__(
    ...
    def get(self, idx):
        ...
        edges_idx_attr_list = torch.cat([data['edge_index'], data['edge_attr'].T], dim=0).T.tolist()
        #print(edges_list)
        ...
        while True:
            rand_edge = random.choice(edges_idx_attr_list)
            if rand_edge[2] == 0:
                i = rand_edge[0]
                j = rand_edge[1]
                adj = np.zeros((num_atoms, num_atoms))
                tmp_list = copy.deepcopy(data['edge_index'].T).tolist()
                #print(tmp_list)
                tmp_list.remove([i,j])
                tmp_list.remove([j,i])
                ...
                if num_components == 2:
                    ...
                    if num_comp1 > 1 and num_comp2 > 1:
        ...
        for i in range(len(data['x'])):
            if frag_adj1[i] == True:
                atom_features_list1.append(datas[i])
            else:
                atom_features_list2.append(datas[i])

        #print(atom_features_list1)
        ...
        for [i,j] in edges_list:
            if [i,j] not in final_list:
                ...
                edge_features_list1.append(attrs[idx3])
                idx3 += 1
            elif frag_adj1[i] == False and frag_adj1[j] == False:
                edge_features_list2.append(attrs[idx3])
                idx3 += 1
            else:
                print('>>>>???????')
        ...
        position_data = data['positions'].tolist()
        
        positions1 = []
        positions2 = []
        for i in range(len(data['positions'])):
            if frag_adj1[i] == True:
                positions1.append(position_data[i])
            else:
                positions2.append(position_data[i])
        
        positions1 = torch.Tensor(positions1)
        positions2 = torch.Tensor(positions2)

        
        data1 = Data(x=x1, mol_id = torch.Tensor([1]), edge_index=edge_index1,
                edge_attr=edge_attr1, positions=positions1, id = torch.Tensor([1]))
        data2 = Data(x=x2, mol_id = torch.Tensor([1]), edge_index=edge_index2,
                edge_attr=edge_attr2, positions=positions2, id = torch.Tensor([1]))
