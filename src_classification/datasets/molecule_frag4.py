...
class MoleculeDatasetFrag4(MoleculeDataset):
    ...
    def __init__(self, root, n_mol,choose = 0,transform=None, seed=777,
        ...
        super(MoleculeDatasetFrag4, self).__init__(
            root, transform, pre_transform, pre_filter)

        if not empty:
            self.data, self.slices = torch.load(self.processed_paths[0])
            self.data1, self.slices1 = torch.load(self.processed_paths[0] + "_0_0")
            self.data2, self.slices2 = torch.load(self.processed_paths[0] + "_0_1")
            self.data11, self.slices11 = torch.load(self.processed_paths[0] + "_0")
            self.data12, self.slices12 = torch.load(self.processed_paths[0] + "_1")
            self.data21, self.slices21 = torch.load(self.processed_paths[0] + "_2")
            self.data22, self.slices22 = torch.load(self.processed_paths[0] + "_3")
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            ...
            # second leaf
            item11, slices11 = self.data11[key], self.slices11[key]
            s = list(repeat(slice(None), item11.dim()))
            s[data11.__cat_dim__(key, item11)] = slice(slices11[idx], slices11[idx+1])
            data11[key] = item11[s]

            item12, slices12 = self.data12[key], self.slices12[key]
            s = list(repeat(slice(None), item12.dim()))
            s[data12.__cat_dim__(key, item12)] = slice(slices12[idx], slices12[idx+1])
            data12[key] = item12[s]

            item21, slices21 = self.data21[key], self.slices21[key]
            s = list(repeat(slice(None), item21.dim()))
            s[data21.__cat_dim__(key, item21)] = slice(slices21[idx], slices21[idx+1])
            data21[key] = item21[s]

            item22, slices22 = self.data22[key], self.slices22[key]
            s = list(repeat(slice(None), item22.dim()))
            s[data22.__cat_dim__(key, item22)] = slice(slices22[idx], slices22[idx+1])
            data22[key] = item22[s]

        if self.aug_mode == 'no_aug':
            n_aug, n_aug1, n_aug2, n_aug11, n_aug12, n_aug21, n_aug22  = 4, 4, 4, 4, 4, 4, 4
            ...
            data11 = self.augmentations[n_aug11](data11.clone())
            data12 = self.augmentations[n_aug12](data12.clone())
            data21 = self.augmentations[n_aug21](data21.clone())
            data22 = self.augmentations[n_aug22](data22.clone())
            ...
            n_aug11 = np.random.choice(3,1)[0]
            n_aug12 = np.random.choice(3,1)[0]
            n_aug21 = np.random.choice(3,1)[0]
            n_aug22 = np.random.choice(3,1)[0]
            ...
            data11 = two_augmentations[n_aug11](data11.clone())
            data12 = two_augmentations[n_aug12](data12.clone())
            data21 = two_augmentations[n_aug21](data21.clone())
            data22 = two_augmentations[n_aug22](data22.clone())
            ...
            raise ValueError
        return data, data1, data2, data11, data12, data21, data22
