    ...
    def process(self):
        ...
        if self.smiles_copy_from_3D_file is None:  # 3D datasets
            ...
            for smiles in tqdm(data_smiles_list):
                ...
                #print(Chem.MolToSmiles(Chem.MolFromSmiles(Chem.MolToSmiles(conformer['rd_mol']))))
                ...
                rdkit_mol = Chem.MolFromSmiles(Chem.MolToSmiles(Chem.MolFromSmiles(smiles)))
                ...
                if mols is None:
                    ...
                    if mols[0] is None or mols[1] is None:
                        ...
                        continue
                    try:
                        mol0_0, mol0_1 = mol_fragment(mols[0])
                        mol1_0, mol1_1 = mol_fragment(mols[1])
                        data0 = mol_to_graph_data(mol0_0)
                        data1 = mol_to_graph_data(mol0_1)
                        data2 = mol_to_graph_data(mol1_0)
                        data3 = mol_to_graph_data(mol1_1)
                        data0_mol_list.append(mol0_0)
                        data1_mol_list.append(mol0_1)
                        data2_mol_list.append(mol1_0)
                        data3_mol_list.append(mol1_1)

                        data0_0 = mol_to_graph_data(mols[0])
                        data0_1 = mol_to_graph_data(mols[1])
                        data0_0_mol_list.append(mols[0])
                        data0_1_mol_list.append(mols[1])
                    except:
                        print('exception')
                        continue
                ...
                data0.mol_id = torch.tensor([mol_idx])
                data0.id = torch.tensor([idx])
                ...
                data3.mol_id = torch.tensor([mol_idx])
                data3.id = torch.tensor([idx])
                data3_list.append(data3)

                data0_0.mol_id = torch.tensor([mol_idx])
                data0_0.id = torch.tensor([idx])
                data0_0_list.append(data0_0)

                data0_1.mol_id = torch.tensor([mol_idx])
                data0_1.id = torch.tensor([idx])
                data0_1_list.append(data0_1)
            ...
            for i in range(len(one_functional_group_list)//2):
                data1 = mol_to_graph_data(one_functional_group_list[2*i])
                data2 = mol_to_graph_data(one_functional_group_list[2*i+1])
                data = mol_to_graph_data(mol_combination(one_functional_group_list[2*i], one_functional_group_list[2*i+1]))
        ...
        data0, slices0 = self.collate(data0_list)
        data1, slices1 = self.collate(data1_list)
        data2, slices2 = self.collate(data2_list)
        data3, slices3 = self.collate(data3_list)

        data0_0, slices0_0 = self.collate(data0_0_list)
        data0_1, slices0_1 = self.collate(data0_1_list)


        data0_smiles_list = [Chem.MolToSmiles(x,True) for x in data0_mol_list]
        data1_smiles_list = [Chem.MolToSmiles(x,True) for x in data1_mol_list]
        data2_smiles_list = [Chem.MolToSmiles(x,True) for x in data2_mol_list]
        data3_smiles_list = [Chem.MolToSmiles(x,True) for x in data3_mol_list]

        data0_0_smiles_list = [Chem.MolToSmiles(x,True) for x in data0_0_mol_list]
        data0_1_smiles_list = [Chem.MolToSmiles(x,True) for x in data0_1_mol_list]

        data0_smiles_series = pd.Series(data0_smiles_list)
        data1_smiles_series = pd.Series(data1_smiles_list)
        data2_smiles_series = pd.Series(data2_smiles_list)
        data3_smiles_series = pd.Series(data3_smiles_list)

        data0_0_smiles_series = pd.Series(data0_0_smiles_list)
        data0_1_smiles_series = pd.Series(data0_1_smiles_list)

        
        data0_smiles_series.to_csv(saver_path + "_0", index=False, header=False)
        data1_smiles_series.to_csv(saver_path + "_1", index=False, header=False)
        data2_smiles_series.to_csv(saver_path + "_2", index=False, header=False)
        data3_smiles_series.to_csv(saver_path + "_3", index=False, header=False)

        data0_0_smiles_series.to_csv(saver_path + "_0_0", index=False, header=False)
        data0_1_smiles_series.to_csv(saver_path + "_0_1", index=False, header=False)

        torch.save((data, slices), self.processed_paths[0])
        torch.save((data0, slices0), self.processed_paths[0] + "_0")
        torch.save((data1, slices1), self.processed_paths[0] + "_1")
        torch.save((data2, slices2), self.processed_paths[0] + "_2")
        torch.save((data3, slices3), self.processed_paths[0] + "_3")

        torch.save((data0_0, slices0_0), self.processed_paths[0] + "_0_0")
        torch.save((data0_1, slices0_1), self.processed_paths[0] + "_0_1")
...
if __name__ == '__main__':
    ...
    if args.sum:
        ...
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_3D_nmol{}_4frag_cut_singlebond'.format(data_folder, n_mol)
        ...
        Molecule3DDatasetFrag(root=root_2d, n_mol=n_mol,
                          smiles_copy_from_3D_file='/home/osikjs/GraphMVP/datasets/molecule_datasets/GEOM_2D_3D_nmol50000_cut_singlebond_manual_1110_sorted_dihedral_from_frag_nohydanchor_0.3/processed/smiles.csv')
