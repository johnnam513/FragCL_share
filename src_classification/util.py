...
def dual_Frag_CL(x, x1, x2, y, y1, y2, args):
    ...
    xy = torch.mm(x, y.transpose(1, 0)).view(B, B, 1, 1)
    xy1 = torch.mm(x, y1.transpose(1, 0)).view(B, B, 1, 1)
    xy2 = torch.mm(x, y2.transpose(1, 0)).view(B, B, 1, 1)
    x1y = torch.mm(x1, y.transpose(1, 0)).view(B, B, 1, 1)
    x1y1 = torch.mm(x1, y1.transpose(1, 0)).view(B, B, 1, 1)
    x1y2 = torch.mm(x1, y2.transpose(1, 0)).view(B, B, 1, 1)
    x2y = torch.mm(x2, y.transpose(1, 0)).view(B, B, 1, 1)
    x2y1 = torch.mm(x2, y1.transpose(1, 0)).view(B, B, 1, 1)
    x2y2 = torch.mm(x2, y2.transpose(1, 0)).view(B, B, 1, 1)

    cat1 = torch.cat([xy, xy1, xy2], dim=-1)
    cat2 = torch.cat([x1y, x1y1, x1y2], dim=-1)
    cat3 = torch.cat([x2y, x2y1, x2y2], dim=-1)
    cat = torch.cat([cat1, cat2, cat3], dim=-2)

    # cat1 = torch.cat([x1y1, x1y2], dim=-1)
    # cat2 = torch.cat([x2y1, x2y2], dim=-1)
    # cat = torch.cat([cat1, cat2], dim=-2)

    cat_2d = torch.max(cat, dim=-1).values
    cat_2d = torch.mean(cat_2d, dim=-1)
    cat_3d = torch.max(cat, dim=-2).values
    cat_3d = torch.mean(cat_3d, dim=-1).transpose(1,0)

    # for i in range(B):
    #     cat_2d[i, i] = torch.mean(cat[i, i, :, :].diag())
    #     cat_3d[i, i] = torch.mean(cat[i, i, :, :].diag())

    logits1 = torch.div(cat_2d, args.T)
    logits2 = torch.div(cat_3d, args.T)
    labels = torch.arange(B).long().to(logits1.device)

    CL_loss_1 = criterion(logits1, labels)
    CL_loss_2 = criterion(logits2, labels)
    pred1 = logits1.argmax(dim=1, keepdim=False)
    pred2 = logits2.argmax(dim=1, keepdim=False)

    CL_acc1 = pred1.eq(labels).sum().detach().cpu().item() * 1. / B
    CL_acc2 = pred2.eq(labels).sum().detach().cpu().item() * 1. / B
